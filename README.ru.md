...
- **📊 Панели статистики**: Отслеживание ежедневных стримов, разницы прироста аудитории, прогнозирование вех, количества треков и объема каталога.
- **⚡ Лента активности (Activity Feed)**: Журнал событий в реальном времени с информацией о снимках, достижениях пользователей и обновлениях каталогов.
...
- **♿ Доступность и клавиатурная навигация (a11y)**: Управление с клавиатуры, сортируемые заголовки таблиц, ARIA-атрибуты, семантические элементы и стили `focus-visible`.
- **🌐 Мультиязычность (i18n)**: Поддержка переключения языков (русский, английский) с защищенным от XSS рендерингом.
...
- **🧹 Очистка устаревших треков**: Треки, отсутствующие в двух снимках подряд, автоматически исключаются для сохранения точности аналитики.
...
- **`GET /api/activity-feed`**: Лента глобальных событий системы и уведомлений о достижениях авторов.
- **`GET /api/track-history`**: Хронология прослушиваний отдельного трека.
- **`GET /api/notification-settings`**: Получение конфигурации публичных уведомлений и вебхуков.
...
- **`POST /api/admin/search-metadata`**: Поиск записей метаданных треков в кэше D1.
- **`POST /api/admin/update-metadata`**: Ручное обновление ISRC, названия или артиста трека.
- **`POST /api/admin/merge-users`**: Объединение дубликатов авторов с консолидацией их истории.
- **`POST /api/admin/export-user`**: Экспорт подробных данных снимков автора для администратора.
- **`POST /api/admin/sync-prod-db`**: Синхронизация данных между боевой и тестовой базой D1.
- **`POST /api/admin/prune-snapshots`**: Очистка устаревших/избыточных снимков для экономии места.
- **`POST /api/admin/notification-settings`**: Сохранение глобальных параметров вебхуков Discord.
...
Это открытый проект с открытым исходным кодом. Вы можете делать вклад в проект! Предпочтительны pull request'ы, но клоны также приветствуются.
